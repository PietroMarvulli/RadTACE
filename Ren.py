...
def best_knn(data,target):
    ...
    for k in tqdm(neighbors_range, desc="Training KNN models"):
        ...
        auc_scores = cross_val_score(knn, data, target, cv=skf, scoring=auc_scorer)
        mean_auc = np.mean(auc_scores)
        if mean_auc > best_auc:
            best_auc = mean_auc
            best_k = k
            best_model = knn
    print(f"\nthe Best Model is {best_k}-NN --> AUC: {best_auc:.4f}")
    return best_model, best_k, best_auc
...
if __name__ == "__main__":
    ...
    radiomics_features.iloc[:,:] = scaler.fit_transform(radiomics_features)
    deep_features.iloc[:,:] = scaler.fit_transform(deep_features)

    radiomics_features = mad_analysis(radiomics_features)
    deep_features = mad_analysis(deep_features)

    radiomics_features = anova_analysis(radiomics_features, target)
    deep_features = anova_analysis(deep_features, target)

    mutual_info = mutual_info_classif(deep_features, target)
    mi_df = pd.DataFrame({'Feature': deep_features.columns,'Mutual Information': mutual_info})
    mi_df = mi_df.sort_values(by='Mutual Information', ascending=False)
    selected_feature = mi_df[mi_df['Mutual Information'] > 0]['Feature']
    deep_features = deep_features[selected_feature]
    best_knn(deep_features, target)
